...
export default function POSSetup() {
  ...
  const [registerForm, setRegisterForm] = useState({ store_id: '', code: '', name: '' });
  const [shiftForm, setShiftForm] = useState({ register_id: '', opening_cash_amount: '0.00' });
  const [providerForm, setProviderForm] = useState({ provider: 'paystack', display_name: '', status: 'active' });
  const [deviceForm, setDeviceForm] = useState({ register_id: '', device_name: '', device_code: '' });
  ...
  const createRegister = useMutation({ mutationFn: () => api.setup.createRegister(compact(registerForm)), onSuccess: () => { setRegisterForm({ store_id: '', code: '', name: '' }); qc.invalidateQueries({ queryKey: ['commerce.registers'] }); toast.success?.('Register created.'); }, onError: (e) => toast.error?.(e?.message || 'Register could not be created.') });
  const openShift = useMutation({ mutationFn: () => api.shifts.open(compact(shiftForm)), onSuccess: () => { setShiftForm({ register_id: '', opening_cash_amount: '0.00' }); qc.invalidateQueries({ queryKey: ['commerce.shifts'] }); toast.success?.('Shift opened.'); }, onError: (e) => toast.error?.(e?.message || 'Shift could not be opened.') });
  const saveProvider = useMutation({ mutationFn: () => api.setup.savePaymentProvider(compact(providerForm)), onSuccess: () => { setProviderForm({ provider: 'paystack', display_name: '', status: 'active' }); qc.invalidateQueries({ queryKey: ['commerce.paymentProviders'] }); toast.success?.('Payment provider saved.'); }, onError: (e) => toast.error?.(e?.message || 'Provider could not be saved.') });
  const registerDevice = useMutation({ mutationFn: () => api.setup.registerDevice(compact(deviceForm)), onSuccess: () => { setDeviceForm({ register_id: '', device_name: '', device_code: '' }); qc.invalidateQueries({ queryKey: ['commerce.devices'] }); toast.success?.('Device registered.'); }, onError: (e) => toast.error?.(e?.message || 'Device could not be registered.') });
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <Panel title="Registers" subtitle="Registers identify tills, counters or mobile checkout points." actions={<button className={btnClass} onClick={() => createRegister.mutate()} disabled={createRegister.isPending}><Plus className="h-4 w-4" /> Add register</button>}>
          <div className="mb-4 grid gap-3 md:grid-cols-3"><Field label="Store"><select className={inputClass} value={registerForm.store_id} onChange={(e) => setRegisterForm((f) => ({ ...f, store_id: e.target.value }))}><option value="">Select store</option>{stores.map((s) => <option value={s.id} key={s.id}>{s.name || s.code}</option>)}</select></Field><Field label="Code"><input className={inputClass} value={registerForm.code} onChange={(e) => setRegisterForm((f) => ({ ...f, code: e.target.value }))} /></Field><Field label="Name"><input className={inputClass} value={registerForm.name} onChange={(e) => setRegisterForm((f) => ({ ...f, name: e.target.value }))} /></Field></div>
          <SimpleTable rows={registers} columns={[{ key: 'code', label: 'Code' }, { key: 'name', label: 'Register' }, { key: 'store_name', label: 'Store', render: (r) => r.store_name || r.store_code || r.store_id || '—' }, { key: 'status', label: 'Status', render: (r) => <Badge>{r.status || 'active'}</Badge> }]} />
        </Panel>
        ...
        <Panel title="Shifts" subtitle="Open shifts before selling and close them with cash counts." actions={<button className={btnClass} onClick={() => openShift.mutate()} disabled={openShift.isPending}><Banknote className="h-4 w-4" /> Open shift</button>}>
          <div className="mb-4 grid gap-3 md:grid-cols-2"><Field label="Register"><select className={inputClass} value={shiftForm.register_id} onChange={(e) => setShiftForm((f) => ({ ...f, register_id: e.target.value }))}><option value="">Select register</option>{registers.map((r) => <option value={r.id} key={r.id}>{r.name || r.code}</option>)}</select></Field><Field label="Opening cash"><input className={inputClass} value={shiftForm.opening_cash_amount} onChange={(e) => setShiftForm((f) => ({ ...f, opening_cash_amount: e.target.value }))} /></Field></div>
          <SimpleTable rows={shifts} columns={[{ key: 'shift_no', label: 'Shift' }, { key: 'register_name', label: 'Register', render: (r) => r.register_name || r.register_id }, { key: 'opened_at', label: 'Opened', render: (r) => dateish(r.opened_at || r.created_at) }, { key: 'status', label: 'Status', render: (r) => <Badge>{r.status}</Badge> }]} />
        </Panel>

        <Panel title="Devices & payments" subtitle="Register cashier devices and configure payment providers.">
          <div className="mb-4 grid gap-3 md:grid-cols-2"><Field label="Provider"><select className={inputClass} value={providerForm.provider} onChange={(e) => setProviderForm((f) => ({ ...f, provider: e.target.value }))}><option value="paystack">Paystack</option><option value="flutterwave">Flutterwave</option><option value="hubtel">Hubtel</option><option value="theteller">TheTeller</option><option value="manual_terminal">Manual terminal</option></select></Field><Field label="Display name"><input className={inputClass} value={providerForm.display_name} onChange={(e) => setProviderForm((f) => ({ ...f, display_name: e.target.value }))} /></Field></div>
          <button className={softBtn} onClick={() => saveProvider.mutate()}><Plus className="h-4 w-4" /> Save provider</button>
          <div className="mt-4"><SimpleTable rows={providers} columns={[{ key: 'provider', label: 'Provider' }, { key: 'display_name', label: 'Name' }, { key: 'status', label: 'Status', render: (r) => <Badge>{r.status}</Badge> }]} /></div>
          <div className="mt-6 grid gap-3 md:grid-cols-3"><Field label="Register"><select className={inputClass} value={deviceForm.register_id} onChange={(e) => setDeviceForm((f) => ({ ...f, register_id: e.target.value }))}><option value="">Select register</option>{registers.map((r) => <option value={r.id} key={r.id}>{r.name || r.code}</option>)}</select></Field><Field label="Device name"><input className={inputClass} value={deviceForm.device_name} onChange={(e) => setDeviceForm((f) => ({ ...f, device_name: e.target.value }))} /></Field><Field label="Device code"><input className={inputClass} value={deviceForm.device_code} onChange={(e) => setDeviceForm((f) => ({ ...f, device_code: e.target.value }))} /></Field></div>
          <button className={`${softBtn} mt-3`} onClick={() => registerDevice.mutate()}><Smartphone className="h-4 w-4" /> Register device</button>
          <div className="mt-4"><SimpleTable rows={devices} columns={[{ key: 'device_code', label: 'Code' }, { key: 'device_name', label: 'Device' }, { key: 'register_id', label: 'Register' }, { key: 'last_seen_at', label: 'Last seen', render: (r) => dateish(r.last_seen_at) }]} /></div>
        </Panel>
      </div>
    </div>
  );
