...
export default function CommerceReturns() {
  ...
  const [returnForm, setReturnForm] = useState({ sale_id: '', reason: '', disposition: 'restock' });
  const [refundForm, setRefundForm] = useState({ sale_id: '', amount: '', method: 'original_payment', reason: '' });
  ...
  const createReturn = useMutation({ mutationFn: () => api.returns.create(compact(returnForm)), onSuccess: () => { setReturnForm({ sale_id: '', reason: '', disposition: 'restock' }); qc.invalidateQueries({ queryKey: ['commerce.returns'] }); toast.success?.('Return request created.'); }, onError: (e) => toast.error?.(e?.message || 'Return could not be created.') });
  const createRefund = useMutation({ mutationFn: () => api.refunds.create(compact(refundForm)), onSuccess: () => { setRefundForm({ sale_id: '', amount: '', method: 'original_payment', reason: '' }); qc.invalidateQueries({ queryKey: ['commerce.refunds'] }); toast.success?.('Refund request created.'); }, onError: (e) => toast.error?.(e?.message || 'Refund could not be created.') });
