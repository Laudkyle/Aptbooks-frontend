...
import { Badge, Field, Panel, SimpleTable, dateish, money, rowsOf } from './_commerceUi.js';
...
export default function CommerceReturns() {
  ...
  const salesQ = useQuery({
    queryKey: ['commerce.pos.sales', 'returns-refunds'],
    queryFn: () => api.pos.sales({}),
  });
  ...
  const returnSaleOptions = [
    { value: '', label: 'Select sale' },
    ...sales
      .filter((sale) => ['completed', 'posted', 'partially_returned'].includes(sale.status))
      .map((sale) => ({ value: sale.id, label: `${sale.sale_no} — ${dateish(sale.sale_date)} — ${money(sale.total_amount)}` })),
  ];
  const refundSaleOptions = [
    { value: '', label: 'Select sale' },
    ...sales
      .filter((sale) => !['draft', 'voided', 'cancelled'].includes(sale.status))
      .map((sale) => ({ value: sale.id, label: `${sale.sale_no} — ${dateish(sale.sale_date)} — ${money(sale.total_amount)}` })),
  ];
      ...
      {tab === 'returns' ? (
        ...
        <div className="grid gap-4 xl:grid-cols-[0.8fr_1.4fr]">
          <Panel title="New refund" subtitle="Refunds require approval before posting." actions={<button className={btnClass} onClick={() => createRefund.mutate()}><Plus className="h-4 w-4" /> Create</button>}>
            <div className="space-y-3"><Field label="Original sale"><select className={inputClass} value={refundForm.saleId} onChange={(e) => setRefundForm((f) => ({ ...f, saleId: e.target.value }))}>{refundSaleOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><Field label="Amount"><input className={inputClass} value={refundForm.amount} onChange={(e) => setRefundForm((f) => ({ ...f, amount: e.target.value }))} /></Field><Field label="Method"><select className={inputClass} value={refundForm.method} onChange={(e) => setRefundForm((f) => ({ ...f, method: e.target.value }))}><option value="original_payment">Original payment</option><option value="cash">Cash</option><option value="momo">Mobile money</option><option value="store_credit">Store credit</option></select></Field><Field label="Reason"><textarea className={inputClass} value={refundForm.reason} onChange={(e) => setRefundForm((f) => ({ ...f, reason: e.target.value }))} /></Field></div>
          </Panel>
