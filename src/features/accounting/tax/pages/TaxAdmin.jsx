...
export default function TaxAdmin() {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {tab === 'settings' ? <ContentCard title="Posting and control settings"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><Select label="Output tax account" value={outputTaxAccountId} onChange={(e) => setOutputTaxAccountId(e.target.value)} options={accountOptions} /><Select label="Input tax account" value={inputTaxAccountId} onChange={(e) => setInputTaxAccountId(e.target.value)} options={accountOptions} /><Select label="Default tax code" value={defaultTaxCodeId} onChange={(e) => setDefaultTaxCodeId(e.target.value)} options={taxCodeOptions} /><Select label="Withholding payable account" value={withholdingPayableAccountId} onChange={(e) => setWithholdingPayableAccountId(e.target.value)} options={accountOptions} /><Select label="Non-recoverable tax account" value={nonRecoverableTaxAccountId} onChange={(e) => setNonRecoverableTaxAccountId(e.target.value)} options={accountOptions} /><div className="flex items-end"><Button onClick={() => saveSettings.mutate()} loading={saveSettings.isPending}>Save settings</Button></div></div><div className="mt-6"><JsonPanel title="Current settings payload" value={settingsQ.data} /></div></ContentCard> : null}
