...
function totalsFrom(data) {
  return data?.data?.totals ?? data?.totals ?? null;
}

function asOfFrom(data) {
  return data?.data?.as_of_date ?? data?.as_of_date ?? null;
}
...
function calculateSummary(rows, apiTotals) {
  if (apiTotals) {
    return {
      ...
      billCount:        rows.reduce((n, v) => n + (v.bills?.length ?? 0), 0),
...
function VendorBillBlock({ vendor }) {
  const [open, setOpen] = useState(true);
  const { vendor_name, vendor_id, buckets, bills = [] } = vendor;
    ...
    <div className="border border-slate-200 rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-purple-50 flex items-center justify-center text-xs font-semibold text-purple-700 flex-shrink-0 select-none">
            {initials(vendor_name)}
          </div>
          <div>
            <p className="text-sm font-semibold text-slate-900">{vendor_name}</p>
            <p className="text-xs text-slate-400 font-mono mt-0.5">{vendor_id?.slice(0, 24)}…</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <table className="w-full text-xs min-w-[640px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {['Bill', 'Bill date', 'Due date', 'Currency', 'Total', 'Outstanding', 'Days past due', 'Bucket'].map(h => (
                  <th
                    key={h}
            ...
            <tbody>
              {bills.map((bill, idx) => (
                <tr
                  key={bill.bill_id}
                  className={`hover:bg-slate-50 transition-colors ${idx < bills.length - 1 ? 'border-b border-slate-50' : ''}`}
                >
                  <td className="px-4 py-2.5">
                    <span className="font-mono font-semibold text-purple-600">{bill.bill_no}</span>
                  </td>
                  <td className="px-4 py-2.5 text-slate-600">{fmtDate(bill.bill_date)}</td>
                  <td className="px-4 py-2.5 text-slate-600">{fmtDate(bill.due_date)}</td>
                  <td className="px-4 py-2.5 text-slate-600">{bill.currency_code}</td>
                  <td className="px-4 py-2.5 text-right font-mono text-slate-700">{formatCurrency(bill.total)}</td>
                  <td className="px-4 py-2.5 text-right font-mono font-semibold text-slate-900">{formatCurrency(bill.outstanding)}</td>
                  <td className="px-4 py-2.5 text-right">
                    {bill.days_past_due != null && <DaysPill days={bill.days_past_due} />}
                  </td>
                  <td className="px-4 py-2.5 text-right">
                    <BucketPill bucket={bill.bucket} />
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function ReportApAging() {
  ...
  const columns = useMemo(() => {
    ...
    return [
      {
        header: 'Vendor', accessor: 'vendor_name',
        render: row => <span className="text-sm font-medium text-slate-900">{row.vendor_name || '—'}</span>,
      },
      {
  ...
  const handleExport = () => {
    ...
    const a    = Object.assign(document.createElement('a'), {
      href: url,
      download: `ap-aging-${asOfDate || new Date().toISOString().split('T')[0]}.csv`,
    });
    ...
    <div className="max-w-[1400px] mx-auto space-y-6 pb-8">
      ...
      {asOf && (
        <div className="flex items-center gap-2 text-xs text-slate-500">
          ...
          &nbsp;·&nbsp;{summary?.totalVendors ?? 0} vendor{summary?.totalVendors !== 1 ? 's' : ''}
          &nbsp;·&nbsp;{summary?.billCount ?? 0} bill{summary?.billCount !== 1 ? 's' : ''}
        </div>
      )}
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 rounded-lg p-4">
            <p className="text-xs font-medium text-slate-500 mb-1">Total vendors</p>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium text-blue-900">Payment rate</p>
          ...
          <div className="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium text-amber-900">Overdue risk</p>
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium text-purple-900">Average per vendor</p>
              <svg className="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <p className="text-2xl font-bold text-purple-900">
              {formatCurrency(summary.totalOutstanding / summary.totalVendors)}
            </p>
            <p className="text-xs text-purple-700 mt-1">Per vendor balance</p>
          </div>
        </div>
      )}
      ...
      {showOverdueAlert && (
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-red-800">Immediate payment action required</p>
            <p className="text-xs text-red-700 mt-0.5">
              Over 50% of outstanding payables ({formatCurrency(summary.days120Plus)}) are 120+ days overdue.
              Review and prioritize payments to avoid vendor relationship issues.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {rows.length} vendor{rows.length !== 1 ? 's' : ''} with outstanding balances · click to expand bills
                </p>
              </div>
            </div>

            {/* expandable per-vendor bill breakdown */}
            <div className="space-y-2 mb-6">
              {rows.map(vendor => (
                <VendorBillBlock key={vendor.vendor_id} vendor={vendor} />
              ))}
            </div>

            {/* summary rollup via DataTable */}
            <div>
              <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Summary by vendor</h4>
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                <DataTable
                  columns={columns}
                  rows={rows}
                  loading={isLoading}
                  empty={{ title: 'No payables found', description: 'Adjust filters and refresh to view data.' }}
                />
              </div>
            </div>
          </>
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-700">
            <div>
              ...
              <p className="text-xs text-slate-500">
                The Accounts Payable Aging report shows how long vendor bills have been outstanding.
