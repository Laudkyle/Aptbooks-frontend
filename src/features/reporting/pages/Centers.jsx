...
export default function Centers() {
  ...
  const columns = useMemo(() => {
    ...
    baseColumns.push({
      header: 'Parent',
      render: (r) => {
        if (!r.parentId) return <span className="text-sm text-text-soft">—</span>;
        const parent = viewMode === 'all' ? rows.find(c => c.id === r.parentId) : allCenters.find(c => c.id === r.parentId);
        return (
          <div className="flex items-center gap-1 text-sm text-text-muted">
            <FolderTree className="h-3.5 w-3.5 text-text-soft" />
    ...
    baseColumns.push({
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          <Building2 className="h-3.5 w-3.5 text-text-soft" />
          <span className="font-mono text-sm text-text-strong">{r.code ?? '—'}</span>
        </div>
      )
    });
    ...
    baseColumns.push({
      ...
      render: (r) => (
        <div className="flex flex-col">
          <span className="font-medium text-sm text-text-strong">{r.name}</span>
          {r.code && <span className="text-xs text-text-muted">{r.code}</span>}
        </div>
      )
    });
      ...
      <div className="space-y-6">
        ...
        <ContentCard>
          <div className="flex items-center justify-center py-12">
            <div className="text-sm text-text-muted">Loading centers...</div>
          </div>
        </ContentCard>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      {viewMode === 'all' && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-surface-1 rounded-xl border border-border-subtle p-4">
            <div className="text-sm text-text-muted">Total Centers</div>
            <div className="text-2xl font-semibold text-text-strong">{stats.total}</div>
          </div>
      ...
      <ContentCard>
        <div className="mb-6 space-y-4">
          ...
          {/* Expandable Filters */}
          {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-surface-2 rounded-lg border border-border-subtle">
              <div className="md:col-span-2">
                <div className="relative">
                  ...
                  {searchQuery && (
                    ...
                    >
                      <X className="h-4 w-4 text-text-soft hover:text-text-muted" />
                    </button>
                  )}
                </div>
              </div>
        ...
        {isError ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <div className="text-sm font-medium text-text-strong">Failed to load centers</div>
          ...
          <div className="text-center py-12">
            ...
            <div className="text-sm font-medium text-text-strong mb-1">No centers found</div>
          ...
          <div className="rounded-xl border border-border-subtle bg-surface-2 p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 text-sm font-medium text-text-strong mb-1">
                  <Lock className="h-4 w-4" />Block Center
                </div>
                <div className="text-xs text-text-muted">Prevent transactions from being posted to this center</div>
              </div>
              <input 
                type="checkbox" 
                className="h-5 w-5 rounded border-border-subtle text-blue-600 focus:ring-blue-500 cursor-pointer" 
