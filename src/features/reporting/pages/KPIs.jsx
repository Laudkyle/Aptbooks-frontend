...
export default function KPIs() {
  ...
  const accounts = rowsFrom(coaQ.data);

  const periodOptions = useMemo(() => [NONE_OPTION, ...toOptions(periods, { valueKey: 'id', label: (p) => `${p.code ?? ''} ${p.name ?? ''}`.trim() || p.id })], [periods]);
  const accountOptions = useMemo(() => [NONE_OPTION, ...toOptions(accounts, { valueKey: 'id', label: (a) => `${a.code ?? ''} ${a.name ?? a.account_name ?? ''}`.trim() || a.id })], [accounts]);
