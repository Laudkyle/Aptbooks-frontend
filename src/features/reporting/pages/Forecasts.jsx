...
export default function Forecasts() {
  ...
  // Fetch forecasts with status filter
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["reporting", "forecasts", { status: statusFilter || undefined }],
    queryFn: () => api.forecasts.list({ status: statusFilter || undefined }),
    staleTime: 30000,
    retry: 2,
  });
  ...
  // Handle forecast actions
  const handleActionClick = useCallback((forecast, action) => {
    setSelectedForecast(forecast);
    setActionType(action);
    setActionModalOpen(true);
  }, []);

  // Archive forecast mutation
  const archiveMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await api.forecasts.archive(selectedForecast.id, { idempotencyKey });
      return response.data;
    },
    onSuccess: () => {
      toast.success("Forecast archived successfully");
      setActionModalOpen(false);
      setSelectedForecast(null);
      setActionType(null);
      qc.invalidateQueries({ queryKey: ["reporting", "forecasts"] });
    },
    onError: (err) => {
  ...
  const activateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Forecast activated successfully");
      setActionModalOpen(false);
      setSelectedForecast(null);
      setActionType(null);
      qc.invalidateQueries({ queryKey: ["reporting", "forecasts"] });
              ...
              <DropdownMenuContent align="end">
                {row.status === "draft" && (
                  <DropdownMenuItem onClick={() => handleActionClick(row, "activate")}>
                    ...
                    <span>Activate Forecast</span>
                  </DropdownMenuItem>
                )}
                {row.status !== "archived" && (
                  <DropdownMenuItem onClick={() => handleActionClick(row, "archive")}>
                    <Archive className="mr-2 h-4 w-4 text-amber-600" />
                    <span>Archive Forecast</span>
                  </DropdownMenuItem>
                )}
                <DropdownMenuItem asChild>
                  <Link to={`/planning/forecasts/${row.id}`} className="flex items-center">
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.name.trim()) {
      errors.name = "Forecast name is required";
    } else if (formData.name.trim().length < 2) {
      errors.name = "Forecast name must be at least 2 characters";
    }
  ...
  const getActionModalContent = useCallback(() => {
    if (!selectedForecast || !actionType) return null;
    ...
    return {
      title: isArchiving ? "Archive Forecast" : "Activate Forecast",
      description: isArchiving 
        ? `Are you sure you want to archive "${selectedForecast.name || selectedForecast.id}"?`
        : `Are you sure you want to activate "${selectedForecast.name || selectedForecast.id}"?`,
      warning: isArchiving 
        ? "Archived forecasts become read-only and cannot be modified."
        : "Active forecasts can be used for scenario planning and comparisons.",
  ...
  }, [selectedForecast, actionType, archiveMutation, activateMutation]);
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              aria-label="Create new forecast"
            >
              New Forecast
            </Button>
          </div>
      ...
      <ContentCard>
        <div className="flex items-end justify-between gap-3 mb-4">
          <div className="w-full max-w-xs">
            <Select
              ...
              aria-label="Filter forecasts by status"
            />
          </div>
        </div>
        ...
        <DataTable
          ...
          empty={{
            ...
            description:
              "Create your first forecast to begin scenario planning and analysis.",
          }}
          aria-label="Forecasts table"
        />
      </ContentCard>
        ...
        <div className="px-6 py-5">
            ...
            {showTopInfo && (
              <div className="mt-2 rounded-xl border border-blue-200 bg-blue-50 p-4">
                <div className="flex items-start gap-3">
                  <CloudSun className="h-5 w-5 text-blue-600 flex-shrink-0 mt-0.5" />
                  <div className="text-sm text-blue-900">
                    <div className="font-medium mb-1">
                      Create a financial forecast
                    </div>
                    <div className="text-blue-700">
                      Set up a forecast for scenario planning with versioning,
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <Input
                label="Forecast Name"
                placeholder="e.g., 2024 Revenue Forecast, Q3 Scenario Planning"
                value={formData.name}
              ...
              <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <ul className="text-xs text-slate-600 space-y-2">
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      Forecast will be created with the specified fiscal year and
                      currency
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-slate-400 mt-1.5 flex-shrink-0" />
                    <span>
                      You can create multiple versions and scenarios after creation
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-slate-400 mt-1.5 flex-shrink-0" />
                  ...
                  <li className="flex items-start gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-slate-400 mt-1.5 flex-shrink-0" />
                    <span>
                      <span className="font-medium">Active:</span> Used for scenario planning, can be archived
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-slate-400 mt-1.5 flex-shrink-0" />
