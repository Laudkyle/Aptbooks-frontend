...
export default function AssetImpair() {
  const { id } = useParams();
  ...
  const [form, setForm] = useState({"periodId": "", "entryDate": "", "impairmentAmount": 0, "impairmentLossAccountId": "", "memo": ""});
  ...
  return (
    <>
      <PageHeader
        title="Impair Fixed Asset"
        subtitle="Recognise an impairment loss."
        actions={
...
<Input label="Impairment amount" type="number" min="0" step="0.01" value={form.impairmentAmount} onChange={(e)=>setForm(s=>({...s, impairmentAmount:e.target.value}))} required />
<Select label="Impairment loss account" value={form.impairmentLossAccountId} onChange={(e)=>setForm(s=>({...s, impairmentLossAccountId:e.target.value}))} options={accountOptions} required />
