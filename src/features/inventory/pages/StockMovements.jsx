...
export default function StockMovements() {
  ...
  const periodsRaw = Array.isArray(periodsQ.data) ? periodsQ.data : periodsQ.data?.data ?? [];
  const periodOptions = useMemo(() => [NONE_OPTION, ...toOptions(periodsRaw, { valueKey: 'id', label: (p) => `${p.code ?? ''} ${p.name ?? ''}`.trim() || p.id })], [periodsRaw]);

  const { data, isLoading, error } = useQuery({ queryKey: ['inventory', 'transactions', { periodId: periodId || undefined }], queryFn: () => api.listTransactions({ periodId: periodId || undefined }) });
