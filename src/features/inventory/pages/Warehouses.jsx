...
export default function Warehouses() {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Address',
        render: (r) => <span className="text-sm text-slate-700">{r.address ?? '—'}</span>
      }
    ],
    []
  );
      ...
      <ContentCard>
        ...
        <div className="mt-3">
          <DataTable
            ...
            rows={filtered}
            isLoading={isLoading}
            empty={{ title: 'No warehouses yet', description: 'Create a warehouse to start recording stock movements.' }}
          />
        </div>
      </ContentCard>
    </div>
