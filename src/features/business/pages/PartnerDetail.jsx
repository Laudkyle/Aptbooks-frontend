...
export default function PartnerDetail() {
  ...
  const [tax, setTax] = useState({
    taxIdNumber: '', vatRegistrationNumber: '', taxRegistrationStatus: 'registered', taxTreatment: '', defaultTaxCodeId: '',
    withholdingEnabled: false, withholdingRate: '', recoverabilityPercent: '', exemptionReasonCode: '', exemptionCertificateNumber: '', exemptionExpiryDate: '', reverseChargeEligible: false,
  ...
  React.useEffect(() => {
    ...
    setEdit({ name: p.name ?? '', code: p.code ?? '', email: p.email ?? '', phone: p.phone ?? '', status: p.status ?? 'active', notes: p.notes ?? '' });
  }, [partner]);
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    setTax({
      taxIdNumber: t.taxIdNumber ?? t.tax_id_number ?? '', vatRegistrationNumber: t.vatRegistrationNumber ?? t.vat_registration_number ?? '', taxRegistrationStatus: t.taxRegistrationStatus ?? t.tax_registration_status ?? 'registered',
      taxTreatment: t.taxTreatment ?? t.tax_treatment ?? '', defaultTaxCodeId: t.defaultTaxCodeId ?? t.default_tax_code_id ?? '', withholdingEnabled: !!(t.withholdingEnabled ?? t.withholding_enabled),
      withholdingRate: t.withholdingRate ?? t.withholding_rate ?? '', recoverabilityPercent: t.recoverabilityPercent ?? t.recoverability_percent ?? '', exemptionReasonCode: t.exemptionReasonCode ?? t.exemption_reason_code ?? '',
      exemptionCertificateNumber: t.exemptionCertificateNumber ?? t.exemption_certificate_number ?? '', exemptionExpiryDate: t.exemptionExpiryDate ?? t.exemption_expiry_date ?? '', reverseChargeEligible: !!(t.reverseChargeEligible ?? t.reverse_charge_eligible),
      taxCountryCode: t.countryCode ?? t.country_code ?? '', taxRegionCode: t.regionCode ?? t.region_code ?? '', placeOfSupplyBasis: t.placeOfSupplyBasis ?? t.place_of_supply_basis ?? '',
      eInvoiceScheme: t.eInvoiceScheme ?? t.einvoice_scheme ?? '', buyerReference: t.buyerReference ?? t.buyer_reference ?? '', filingCurrency: t.filingCurrency ?? t.filing_currency ?? ''
    });
  }, [taxProfile]);
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <Modal open={taxOpen} title="Edit tax profile" onClose={() => setTaxOpen(false)} footer={<div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setTaxOpen(false)}>Cancel</Button><Button onClick={() => saveTax.mutate(buildPartnerTaxProfilePayload(tax))}>Save</Button></div>}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><Input label="Tax ID number" value={tax.taxIdNumber} onChange={(e) => setTax((s) => ({ ...s, taxIdNumber: e.target.value }))} /><Input label="VAT registration" value={tax.vatRegistrationNumber} onChange={(e) => setTax((s) => ({ ...s, vatRegistrationNumber: e.target.value }))} /><Input label="Registration status" value={tax.taxRegistrationStatus} onChange={(e) => setTax((s) => ({ ...s, taxRegistrationStatus: e.target.value }))} /><Input label="Tax treatment" value={tax.taxTreatment} onChange={(e) => setTax((s) => ({ ...s, taxTreatment: e.target.value }))} /><Input label="Default tax code" value={tax.defaultTaxCodeId} onChange={(e) => setTax((s) => ({ ...s, defaultTaxCodeId: e.target.value }))} /><Input label="Withholding rate" type="number" value={tax.withholdingRate} onChange={(e) => setTax((s) => ({ ...s, withholdingRate: e.target.value }))} /><Input label="Recoverability (%)" type="number" value={tax.recoverabilityPercent} onChange={(e) => setTax((s) => ({ ...s, recoverabilityPercent: e.target.value }))} /><Input label="Exemption reason" value={tax.exemptionReasonCode} onChange={(e) => setTax((s) => ({ ...s, exemptionReasonCode: e.target.value }))} /><Input label="Exemption certificate" value={tax.exemptionCertificateNumber} onChange={(e) => setTax((s) => ({ ...s, exemptionCertificateNumber: e.target.value }))} /><Input label="Exemption expiry" type="date" value={tax.exemptionExpiryDate} onChange={(e) => setTax((s) => ({ ...s, exemptionExpiryDate: e.target.value }))} /><Input label="Country code" value={tax.taxCountryCode} onChange={(e) => setTax((s) => ({ ...s, taxCountryCode: e.target.value }))} /><Input label="Region code" value={tax.taxRegionCode} onChange={(e) => setTax((s) => ({ ...s, taxRegionCode: e.target.value }))} /><Input label="Place of supply basis" value={tax.placeOfSupplyBasis} onChange={(e) => setTax((s) => ({ ...s, placeOfSupplyBasis: e.target.value }))} /><Input label="E-invoice scheme" value={tax.eInvoiceScheme} onChange={(e) => setTax((s) => ({ ...s, eInvoiceScheme: e.target.value }))} /><Input label="Buyer reference" value={tax.buyerReference} onChange={(e) => setTax((s) => ({ ...s, buyerReference: e.target.value }))} /><Input label="Filing currency" value={tax.filingCurrency} onChange={(e) => setTax((s) => ({ ...s, filingCurrency: e.target.value }))} /><label className="flex items-center gap-2 rounded-xl border border-border-subtle px-3 py-2 text-sm"><input type="checkbox" checked={tax.withholdingEnabled} onChange={(e) => setTax((s) => ({ ...s, withholdingEnabled: e.target.checked }))} /> Withholding enabled</label><label className="flex items-center gap-2 rounded-xl border border-border-subtle px-3 py-2 text-sm"><input type="checkbox" checked={tax.reverseChargeEligible} onChange={(e) => setTax((s) => ({ ...s, reverseChargeEligible: e.target.checked }))} /> Reverse charge eligible</label></div></Modal>
