...
const toast = useToast();
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="grid gap-3 md:grid-cols-2">
          <Input label="Label" value={addressEdit.label} onChange={(e) => setAddressEdit((s) => ({ ...s, label: e.target.value }))} />
          <Input label="Line 1" value={addressEdit.line1} onChange={(e) => setAddressEdit((s) => ({ ...s, line1: e.target.value }))} />
          <Input label="Line 2" value={addressEdit.line2} onChange={(e) => setAddressEdit((s) => ({ ...s, line2: e.target.value }))} />
          <Input label="City" value={addressEdit.city} onChange={(e) => setAddressEdit((s) => ({ ...s, city: e.target.value }))} />
          <Input label="Region" value={addressEdit.region} onChange={(e) => setAddressEdit((s) => ({ ...s, region: e.target.value }))} />
          <Input label="Postal code" value={addressEdit.postalCode} onChange={(e) => setAddressEdit((s) => ({ ...s, postalCode: e.target.value }))} />
          <Input label="Country" value={addressEdit.country} onChange={(e) => setAddressEdit((s) => ({ ...s, country: e.target.value }))} />
          <label className="flex items-center gap-2 rounded-xl border border-border-subtle bg-white/70 p-3 text-sm text-slate-700 md:col-span-2">
            <input
              type="checkbox"
              checked={addressEdit.isPrimary}
              onChange={(e) => setAddressEdit((s) => ({ ...s, isPrimary: e.target.checked }))}
            />
            Primary address
          </label>
        </div>
          ...
          <Button
            ...
            onClick={() =>
              updateAddress.mutate({
                ...
                body: {
                  label: addressEdit.label || undefined,
                  line1: addressEdit.line1 || undefined,
                  line2: addressEdit.line2 || undefined,
                  city: addressEdit.city || undefined,
                  region: addressEdit.region || undefined,
                  postalCode: addressEdit.postalCode || undefined,
                  country: addressEdit.country || undefined,
                  isPrimary: addressEdit.isPrimary
                }
              })
            }
            disabled={!editingAddressId || !addressEdit.line1.trim()}
          >
            Save
          </Button>
        </div>
      </Modal>
      <JsonPaneluseState } from 'react';
import { useParams, Link } from 'react-router-dom';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { ArrowLeft, Building2, CreditCard, MapPin, Phone, Plus, Save } from 'lucide-react';
