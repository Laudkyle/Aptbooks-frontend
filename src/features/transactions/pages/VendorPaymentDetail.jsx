...
export default function VendorPaymentDetail() {
  ...
  const run = useMutation({
    mutationFn: async () => {
      if (action === 'auto') return api.autoAllocate(id, { rule }, { idempotencyKey });
      if (action === 'reallocate') return api.reallocate(id, allocations, { idempotencyKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={payment?.paymentNumber ?? payment?.code ?? (isLoading ? 'Loading payment…' : 'Payment')}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <ContentCard className="lg:col-span-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500">Vendor, cash account, amount and allocations.</div>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              ...
              <div className="mt-1 text-sm font-semibold text-slate-900">{payment?.vendorName ?? payment?.vendorId ?? '—'}</div>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              <div className="text-xs text-slate-500">Payment date</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{payment?.paymentDate ?? '—'}</div>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              <div className="text-xs text-slate-500">Cash account</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{payment?.cashAccountId ?? '—'}</div>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              <div className="text-xs text-slate-500">Amount total</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{payment?.amountTotal ?? '—'}</div>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-border-subtle bg-white/70">
                  {(payment?.allocations ?? []).map((a, idx) => (
                    <tr key={idx}>
                      <td className="px-4 py-3">{a.billId ?? '—'}</td>
        ...
        <ContentCard>
          <JsonPanel title="Payment (GET)" value={payment ?? {}} />
        </ContentCard>
      </div>

      <Modal open={!!action} onClose={() => setAction(null)} title={action === 'auto' ? 'Auto-allocate' : action === 'reallocate' ? 'Reallocate' : action === 'post' ? 'Post payment' : action === 'void' ? 'Void payment' : 'Action'}>
        <div className="space-y-3">
          <Input label="Idempotency-Key (required)" value={idempotencyKey} onChange={(e) => setIdempotencyKey(e.target.value)} placeholder="uuid…" />
          {action === 'auto' ? (
          ...
          {action === 'reallocate' ? (
            <JsonPanel
              ...
              value={allocations}
              hint="Shape: { allocations: [ { billId, amountApplied } ] }"
          ...
          {action === 'post' ? (
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4 text-xs text-slate-600">
              Posting finalises the payment and books it to the ledger.
            </div>
          ) : null}
        </div>
