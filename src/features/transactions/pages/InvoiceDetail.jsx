...
export default function InvoiceDetail() {
  ...
  const applyCreditNote = useMutation({
    ...
    onError: (e) =>
      toast.error(
        e?.response?.data?.error ?? e?.message ?? "Failed to apply credit note",
      ),
  });
  ...
  const status =
    workflowState.businessStatus || invoice?.workflow_status || "draft";
  ...
  const creditNotesRaw = Array.isArray(creditNotesQ.data)
    ? creditNotesQ.data
    : (creditNotesQ.data?.data ?? []);
  const availableCreditNotes = creditNotesRaw.filter((note) => {
    ...
    return (
      (!customerId || noteCustomerId === customerId) &&
      remaining > 0 &&
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2 rounded-lg border border-gray-200 p-4">
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    {invoice?.approved_at && (
                      <div>Approved: {formatDate(invoice.approved_at)}</div>
                    )}
                    {invoice?.issued_at && (
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {withholdingTotal > 0 ? (
                <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                  ...
                  <p className="mt-1">
                    Customer receipts should settle the net collectible only. The withheld portion should be cleared later from the withholding workspace through a certificate or reconciliation.
                  </p>
              ...
              <div className="mt-4 grid gap-2">
                ...
                >
                  {withholdingTotal > 0 ? "Receive Net Payment" : "Receive Payment"}
                </Button>
                <Button
                  variant="outline"
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Use issued credit notes to reduce this invoice without creating
                a cash receipt.
              </p>
              {!canApplyCredits ? (
                <div className="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-600">
                  Credit notes can be applied only when the invoice is issued
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              navigate(ROUTES.creditNoteDetail(note.id))
                            }
                            className="border-gray-300"
                          >
                            Open
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                {lines.map((line, idx) => {
                  ...
                  if (
                    taxBreakdown?.components?.length &&
                    taxBreakdown.components[0]?.tax_amount > 0
                  ) {
