...
export default function BillDetail() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={bill?.billNumber ?? bill?.code ?? (isLoading ? 'Loading bill…' : 'Bill')}
        subtitle={`Bill ID: ${id}`}
        icon={FileText}
        actions={
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <ContentCard className="lg:col-span-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500">Dates, vendor reference and memo.</div>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              ...
              <div className="mt-1 text-sm font-semibold text-slate-900">{bill?.vendorName ?? bill?.vendorId ?? '—'}</div>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
            ...
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4 md:col-span-2">
              ...
              <div className="mt-1 text-sm text-slate-700">{bill?.memo ?? <span className="text-slate-400">—</span>}</div>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-border-subtle bg-white/70">
                  {(bill?.lines ?? []).map((l, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-4 py-3">{l.expenseAccountId ?? '—'}</td>
                    </tr>
                  ))}
                  {!(bill?.lines ?? []).length ? (
                    <tr>
        ...
        <ContentCard>
          ...
          <div className="mt-3">
            <JsonPanel title="Bill (GET)" value={bill ?? {}} />
          </div>
        </ContentCard>
      </div>
