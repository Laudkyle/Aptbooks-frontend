...
export default function BillDetail() {
  ...
  const { data, isLoading } = useQuery({ queryKey: qk.bill(id), queryFn: () => api.get(id) });
  const billData = data?.data ?? data;
  const bill = billData?.bill || billData;
  const lines = billData?.lines || [];
  const allocationsSummary = billData?.allocations_summary || {};
  const paid = allocationsSummary?.paid || 0;
  const outstanding = allocationsSummary?.outstanding || bill?.total || 0;
  ...
  const applyDebitNote = useMutation({
    ...
    onError: (e) => toast.error(e?.response?.data?.error ?? e?.message ?? 'Failed to apply debit note')
  });
  ...
  const debitNotesRaw = Array.isArray(debitNotesQ.data) ? debitNotesQ.data : debitNotesQ.data?.data ?? [];
...
console.log(availableDebitNotes)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-2xl font-bold text-gray-900">{bill?.bill_no || (isLoading ? 'Loading...' : 'Bill')}</h1>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-lg border border-gray-200 p-4"><div className="flex items-center gap-2 mb-2"><User className="h-4 w-4 text-gray-400" /><span className="text-xs font-medium text-gray-500">Vendor ID</span></div><div className="text-sm font-semibold text-gray-900">{vendorId || '—'}</div></div>
                <div className="rounded-lg border border-gray-200 p-4"><div className="flex items-center gap-2 mb-2"><Calendar className="h-4 w-4 text-gray-400" /><span className="text-xs font-medium text-gray-500">Dates</span></div><div className="text-sm font-semibold text-gray-900">{formatDate(bill?.bill_date)} → {formatDate(bill?.due_date)}</div></div>
                {bill?.memo && <div className="md:col-span-2 rounded-lg border border-gray-200 p-4"><div className="text-xs font-medium text-gray-500 mb-2">Memo</div><div className="text-sm text-gray-700">{bill.memo}</div></div>}
                <div className="md:col-span-2 rounded-lg border border-gray-200 p-4"><div className="text-xs font-medium text-gray-500 mb-2">Timeline</div><div className="space-y-1 text-sm"><div>Created: {formatDate(bill?.created_at)}</div>{bill?.submitted_at && <div>Submitted: {formatDate(bill.submitted_at)}</div>}{bill?.approved_at && <div>Approved: {formatDate(bill.approved_at)}</div>}</div></div>
              </div>
            </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center gap-2 mb-4"><DollarSign className="h-5 w-5 text-green-600" /><h3 className="text-base font-semibold text-gray-900">Bill Total</h3></div>
              ...
              <div className="mt-4 grid gap-2">
                <Button onClick={() => navigate(ROUTES.vendorPaymentNew)} className="bg-green-600 hover:bg-green-700">Pay Vendor</Button>
                <Button variant="outline" onClick={() => navigate(ROUTES.debitNoteNew)} className="border-gray-300">New Debit Note</Button>
              </div>
            </div>

            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center gap-2 mb-4"><FileMinus2 className="h-5 w-5 text-amber-600" /><h3 className="text-base font-semibold text-gray-900">Apply Debit Note</h3></div>
              <p className="text-sm text-gray-600 mb-4">Use issued debit notes to reduce this bill without creating a cash payment.</p>
              {!canApplyDebits ? (
                <div className="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-600">Debit notes can be applied only when the bill is issued and still has an outstanding balance.</div>
              ) : debitNotesQ.isLoading ? (
                <div className="text-sm text-gray-500">Loading debit notes...</div>
                ...
                <div className="space-y-3">
                  ...
                  <Button variant="outline" onClick={() => navigate(ROUTES.debitNoteNew)} className="border-gray-300 w-full">Create Debit Note</Button>
                </div>
              ) : (
                <div className="space-y-3">
                    ...
                    return (
                      <div key={note.id} className="rounded-lg border border-gray-200 p-4 space-y-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Button variant="outline" size="sm" onClick={() => navigate(ROUTES.debitNoteDetail(note.id))} className="border-gray-300">Open</Button>
                        </div>
                        <Input type="number" step="0.01" min="0" value={amount} onChange={(e) => setNoteAmounts((prev) => ({ ...prev, [note.id]: e.target.value }))} />
                        <Button
                          onClick={() => applyDebitNote.mutate({ debitNoteId: note.id, amount: Number(amount) || 0 })}
                          disabled={applyDebitNote.isPending || !(Number(amount) > 0) || Number(amount) > remaining || Number(amount) > outstandingAmount}
      ...
      <Modal open={!!action} onClose={() => setAction(null)} title={action === 'submit' ? 'Submit for Approval' : action === 'approve' ? 'Approve Bill' : action === 'reject' ? 'Reject Bill' : action === 'issue' ? 'Issue Bill' : action === 'void' ? 'Void Bill' : 'Action'}>
        <div className="space-y-4">
          ...
          {action === 'void' && <div><label className="block text-sm font-medium text-gray-700 mb-2">Reason <span className="text-red-500">*</span></label><Textarea rows={4} value={voidReason} onChange={(e) => setVoidReason(e.target.value)} placeholder="Explain why this bill is being voided..." /></div>}
