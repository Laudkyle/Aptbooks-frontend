        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-surface-1 rounded-lg shadow-sm border border-border-subtle p-6">
              <h3 className="text-base font-semibold text-text-strong mb-5">Bill Summary</h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                {bill?.bill?.memo && (
                  <div className="md:col-span-2  rounded-lg border border-border-subtle p-4">
                    ...
                    <div className="text-sm text-text-body">{bill.bill.memo}</div>
                  </div>
                )}
              </div>
            </div>
                ...
                <table className="w-full">
                  ...
                  <tbody className="bg-surface-1 divide-y divide-border-subtle">
                    {(bill?.lines ?? []).map((l, idx) => {
                      const lineTotal = (l.quantity ?? 1) * (l.unitPrice ?? l.unit_price ?? 0);
          ...
          <div className="lg:col-span-1">
            <div className="bg-surface-1 rounded-lg shadow-sm border border-border-subtle p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base font-semibold text-text-strong">Bill Total</h3>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-border-subtle">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-text-strong">{(bill?.lines ?? []).length}</span>
                </div>
                {(bill?.bill?.billDate || bill?.bill?.bill_date) && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-muted">Bill Date</span>
                    <span className="font-medium text-text-strong">{formatDate(bill?.bill?.billDate) || formatDate(bill?.bill?.bill_date)}</span>
                  </div>
                )}
                {(bill?.bill?.dueDate || bill?.bill?.due_date) && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-muted">Due Date</span>
                    <span className="font-medium text-text-strong">{formatDate(bill?.bill?.dueDate) || formatDate(bill?.bill?.due_date)}</span>
                  </div>
                )}
              </div>
            </div>
