...
export default function CustomerReceiptList() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: qk.customerReceipts(qs),
    queryFn: () => api.list(qs)
  });
    ...
    () => [
      {
        ...
        render: (r) => (
          <div className="flex flex-col">
            <Link to={ROUTES.customerReceiptDetail(r.id)} className="font-medium text-brand-deep hover:underline">
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        icon={HandCoins}
        actions={
          <Button leftIcon={Plus} onClick={() => navigate(ROUTES.customerReceiptNew)}>
            New receipt
          </Button>
        }
      />
