...
export default function DebitNoteDetail() {
  ...
  const [applyBody, setApplyBody] = useState({ billId: '', amountApplied: 0 });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={note?.debitNoteNumber ?? note?.code ?? (isLoading ? 'Loading debit note…' : 'Debit note')}
        ...
        <ContentCard className="lg:col-span-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500">Vendor and memo.</div>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              ...
              <div className="mt-1 text-sm font-semibold text-slate-900">{note?.customerName ?? note?.vendorId ?? '—'}</div>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4">
              <div className="text-xs text-slate-500">Date</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{note?.debitNoteDate ?? '—'}</div>
            </div>
          </div>

          <div className="mt-5">
            <JsonPanel title="Debit note (GET)" value={note ?? {}} />
          </div>
        </ContentCard>
      ...
      <Modal open={!!action} onClose={() => setAction(null)} title={action === 'issue' ? 'Issue debit note' : action === 'apply' ? 'Apply to invoice' : action === 'void' ? 'Void debit note' : 'Action'}>
        <div className="space-y-3">
          {action === 'apply' ? (
            <>
              <Input label="Invoice ID" value={applyBody.billId} onChange={(e) => setApplyBody((s) => ({ ...s, billId: e.target.value }))} placeholder="uuid…" />
              <Input
          ...
          {action === 'issue' ? (
            <div className="rounded-2xl border border-border-subtle bg-white/70 p-4 text-xs text-slate-600">
              Issuing makes the debit note available for application.
            </div>
          ) : null}
        </div>
