...
export default function PaymentRunDetailPage(){
  ...
  return <>
    ...
    {run.status==='draft' && <ContentCard className="mt-4"><div className="text-base font-semibold mb-3">Add line</div><div className="grid md:grid-cols-2 gap-4"><Input label="Payee name" value={line.payeeName} onChange={(e)=>setLine(s=>({...s,payeeName:e.target.value}))} /><Select label="Offset account" value={line.offsetAccountId} onChange={(e)=>setLine(s=>({...s,offsetAccountId:e.target.value}))} options={[{value:'', label:'Select account'}, ...coaRows.map(a=>({ value:a.id, label:`${a.code || ''} ${a.name || a.account_name || ''}`.trim() }))]} /><Input label="Amount" type="number" value={line.amount} onChange={(e)=>setLine(s=>({...s,amount:e.target.value}))} /><Input label="Currency code" value={line.currencyCode} onChange={(e)=>setLine(s=>({...s,currencyCode:e.target.value}))} /><Textarea label="Description" value={line.description} onChange={(e)=>setLine(s=>({...s,description:e.target.value}))} className="md:col-span-2" /></div><div className="mt-4 flex justify-end"><Button disabled={addLine.isLoading} onClick={()=>addLine.mutate()}>{addLine.isLoading?'Adding...':'Add line'}</Button></div></ContentCard>}
  </>;
}
